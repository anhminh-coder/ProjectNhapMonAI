...
def face_recognition(image_name):
    ...
    for (x, y, w, h) in faces:
        ...
        print(np.average(dist[0]))
        if(np.average(dist[0])>25):
            checkOutlier = True
        if checkOutlier == True:
            cv2.putText(img_test, "{}".format('Other'), (x, y), font,
    ...
    cv2.imwrite(os.getcwd()+'/result_image/'+image_name, img_test)
